...
for (row in columns) {
    ...
    for (let i=0; i < symbols_duplicate.length; i++) {
        ...
        symbol.position.y = (i - Math.floor(symbols_duplicate.length/2.5))*symbol.height + (frame.position.y - frame_height/2) + symbols_offset;
...
tick.add((delta) => {
    if (move_mode) {
        ...
        for (slot_element in l_col.children) {
            l_col.children[slot_element].position.y = l_col.children[slot_element].position.y - delta*step*8;
        ...
        for (slot_element in m_col.children) {
            m_col.children[slot_element].position.y = m_col.children[slot_element].position.y + delta*step*8;
        ...
        for (slot_element in r_col.children) {
            r_col.children[slot_element].position.y = r_col.children[slot_element].position.y - delta*step*8;
            ...
            for (symbol in l_col.children) {
                for (col in columns) {
                    ...
                    for (position_y in default_positions) {
                        differences.push(Math.abs(columns[col].children[symbol].position.y - default_positions[position_y]))
                    }
                    let min_value = Math.min(...differences)
                    index_min = differences.indexOf(min_value)
                    columns[col].children[symbol].position.y = default_positions[index_min]
                }
            }

            // Mechanism to check which symbols "win"
            for (symbol in l_col.children) {
                if (l_col.children[symbol].position.y == default_positions[Math.floor(symbols.length/2)]) {
                ...
                if (m_col.children[symbol].position.y == default_positions[Math.floor(symbols.length/2)]) {
                ...
                if (r_col.children[symbol].position.y == default_positions[Math.floor(symbols.length/2)]) {
