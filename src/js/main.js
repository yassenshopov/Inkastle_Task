...
tick.add((delta) => {
  if (move_mode) {
    ...
    for (slot_element in l_row.children) {
      l_row.children[slot_element].position.y =
        l_row.children[slot_element].position.y - delta * step * 8;
    ...
    for (slot_element in m_row.children) {
      m_row.children[slot_element].position.y =
        m_row.children[slot_element].position.y + delta * step * 8;
    ...
    for (slot_element in r_row.children) {
      r_row.children[slot_element].position.y =
        r_row.children[slot_element].position.y - delta * step * 8;
...
function play() {
  ...
  if (play_mode == true) {
    ...
    setTimeout(() => {
      ...
      for (symbol in l_row.children) {
        for (row in rows) {
          ...
          for (position_y in default_positions) {
            differences.push(
              Math.abs(
                rows[row].children[symbol].position.y -
      ...
      for (symbol in l_row.children) {
        if (
          l_row.children[symbol].position.y ==
        ...
        if (
          m_row.children[symbol].position.y ==
          default_positions[Math.floor(symbols.length / 2)]
        ) {
        ...
        if (
          r_row.children[symbol].position.y ==
          default_positions[Math.floor(symbols.length / 2)]
        ) {
